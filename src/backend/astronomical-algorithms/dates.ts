...
export function isDateAfterPapalReform(
  year: number,
  Month: number,
  Day: number
): boolean {
...
export function getFullScaleJulianDay(
  year: number,
  Month: number,
  Day: number,
...
export function isLeapYear(year: number, isGregorianCalendar = true): boolean {
  if (isGregorianCalendar) {
    if (year % 100 === 0) {
      return year % 400 === 0;
    } else {
...
export function fractionalYear(jd: number, isGregorianCalendar = true): number {
